...
function getCropRecommendation({ location = '', season = '', soilCondition = '', landSize = '1', priority = 'yield', farmerContext = {}, language = 'English' }) {
  ...
  // Find regional match if available (sort by length descending so 'north-west' precedes 'north')
  const sortedRegKeys = Object.keys(CAMEROON_REGIONS_PROFILE).sort((a, b) => b.length - a.length);
  let matchedRegionKey = null;
  for (const reg of sortedRegKeys) {
  ...
  // Real compatible alternative crops cultivated in this Cameroon region
  const compatibleCrops = sortedCrops.slice(1, 5).map(cropKey => {
    const c = CROPS_RECOMMENDATION_DATA[cropKey];
    return {
      id: c.id,
      name: c.name,
      yield: c.expectedYield,
      maturity: c.maturityDays,
      spacing: c.spacing,
      score: scores[cropKey],
      compatibility: scores[cropKey] >= 85 ? 'Optimal (95%+)' : scores[cropKey] >= 70 ? 'Very High (85%)' : 'Good (75%)'
    };
  });
  ...
  let minYield = 3.5;
  let maxYield = 7.0;
  const yieldMatches = (primary.expectedYield || '').match(/([\d\.]+)\s*-\s*([\d\.]+)/);
  if (yieldMatches && yieldMatches.length >= 3) {
    minYield = parseFloat(yieldMatches[1]) || 3.5;
    maxYield = parseFloat(yieldMatches[2]) || 7.0;
  }
  ...
  const landEstimateText = isFrench
    ? `Pour ${landSize} Hectare(s), le rendement projeté pour ${primary.name} est de ${(sizeNum * minYield).toFixed(1)} - ${(sizeNum * maxYield).toFixed(1)} Tonnes selon la gestion recommandée.`
    : `For ${landSize} Hectare(s), projected yield for ${primary.name} is ${(sizeNum * minYield).toFixed(1)} - ${(sizeNum * maxYield).toFixed(1)} Tons under recommended management.`;
