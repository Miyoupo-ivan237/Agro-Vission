...
const DISEASE_NAME_TRANSLATIONS = {
  ...
  'Fall Armyworm Infestation': 'Infestation de la Légionnaire d\'Automne du Maïs',
  'Maize Common Rust': 'Rouille Commune du Maïs',
  'Northern Corn Leaf Blight (NCLB)': 'Brûlure des Feuilles du Maïs (NCLB)',
  ...
  'Plantain Black Sigatoka': 'Sigatoka Noire du Plantain',
  'Banana & Plantain Weevil Borer': 'Charançon du Bananier et du Plantain',
  'Cocoa Black Pod Disease': 'Maladie de la Pourriture Noire du Cacao',
  'Cocoa Mirids / Capsids Attack': 'Atteinte des Mirides / Capsides du Cacao',
  'Coffee Berry Disease (CBD)': 'Maladie des Baies du Café (CBD)',
  'Coffee Leaf Rust': 'Rouille des Feuilles du Café',
  'Yam Anthracnose / Dieback': 'Anthracnose / Dessèchement de l\'Igname',
  'Groundnut Rosette Virus Disease (GRVD)': 'Maladie de la Rosette de l\'Arachide (GRVD)',
  'Rice Blast Disease': 'Maladie du Pyricladosporiose du Riz',
  'Irish Potato Late Blight': 'Mildiou Tardif de la Pomme de Terre',
  'Pepper Fruit Anthracnose': 'Anthracnose des Fruits du Piment',
  'Oil Palm Basal Stem Rot': 'Pourriture Basale du Stipe du Palmier à Huile',
  'Cotton Bacterial Blight / Black Arm': 'Brûlure Bactérienne / Black Arm du Coton',
  'Onion Purple Blotch': 'Tache Pourpre de l\'Oignon',
  'Okra Yellow Vein Mosaic Virus (OYVMV)': 'Virus de la Mosaïque des Veines Jaunes du Gombo (OYVMV)'
};
...
function translateDiseaseName(name) {
  if (!name) return name;
  const exact = DISEASE_NAME_TRANSLATIONS[name] || DISEASE_NAME_TRANSLATIONS[name.trim()];
  if (exact) return exact;

  const normalized = name.toLowerCase();
  for (const [key, value] of Object.entries(DISEASE_NAME_TRANSLATIONS)) {
...
function translateTreatment(text) {
  ...
  for (const [en, fr] of Object.entries(TREATMENT_TRANSLATIONS)) {
    translated = translated.replace(new RegExp(escapeRegExp(en), 'gi'), fr);
  }
  return translated;
}
